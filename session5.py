...
def squared_power_list(*fn_args, **fn_kwargs):
    ...
    if start == end:
        ...
        Error = ValueError('Squared Power Error - end cannot be smaller than start')
        raise Error
...
def polygon_area(*fn_args, **fn_kwargs):
    ...
    if side <= 0:
        raise ValueError('Poly Area - Side length should be greater than 0')
    ...
    if num_sides == 3:
        ...
        raise ValueError('Poly Area - Area calculation supported only for polygons with sides 3 to 6')
    return area

def temp_converter(*fn_args, **fn_kwargs):
    temp_in = fn_args[0]
    in_scale = fn_kwargs['temp_given_in'].lower()
    ...
    if in_scale == 'f':
        ...
        raise ValueError('Temp Convert - Give Valid Scale, Only c/C and k/K allowed')
